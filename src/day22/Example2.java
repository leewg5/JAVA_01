...
public class Example2 {
    public static void main(String[] args) {
        ...
        // 6. 임의 여러개 데이터중 하나 선택 하기
        ArrayList<String> nameList = new ArrayList<>();
        nameList.add("유재석"); nameList.add("강호동"); nameList.add("신동엽");
        random.nextInt(nameList.size());  // .size(): arrayList 길이 변환 메소드
        ...
        //활용4] 주사위 : 1~6사이
        int dice = random.nextInt(6) + 1;  System.out.println(dice);

        //활용5] 로또 번호 생성 : 1 ~ 45 사이의 중복없이 6번 생성
        ArrayList<Integer>lottoList = new ArrayList<>();
        for (int i =1; i<=6; i++){
            int lotto = random.nextInt(45) + 1;
            if(lottoList.contains(lotto)){
        ...
        String ranStr2 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789가나다라";

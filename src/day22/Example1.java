...
public class Example1 {
    ...
    public static void main(String[] args) {
        ...
        String str7 = "유재석";              // 문자열이 동일할 경우 동일한 객체 취급
        String str8 = new String("유재석");  // new String 사용시 동일한 문자열 이라도 다른객체 취급

        //4. == VS .equals(), == 주소값(객체가 저장된위치) 비교 , .equals() : 객체내 값 비교
        ...
        //5) .charAt(인덱스): 지정한 인덱스 번호의 문자 1개 추출
        char gender = "012345-1234234".charAt(7);
        System.out.println(gender); //1
        // 활용] nextChar()[x]--> 문자1개입력 : scan.next().charAt(0)

        // 6) .length() : 문자열내 문자수 반환 메소드
        System.out.println("012345-123455678".length()); //인덱스:0~13, 길이 14
        // 활용] for(int index= 0 ; index < 문자열.length() ; index++){}

        // 7) .replace(기존 문자열, 새로운 문자열) : 지정한 문자열내 기존문자열을 새로운문자열 생성후 반환 메소드
        ...
        String str13 = "012345-123425454".substring(0,6); //0번 인덱스 부터 6번 인덱스 전 까지
        System.out.println(str13); // 012345
        String str14 = "012345-1234555".substring(7);  //7번 인덱스부터(마지막 인덱스)까지 추출
        System.out.println(str14); // 1234555
        //활용] 차량번호 조회 : 214가 7531 , 차량번호 뒷자리 : 차량번호.subString(차량번호.length()-1-3);

        // 9) .split("구분문자"); : 지정한 문자열내 구분문자 기준으로 잘라서 배열로 반환 메소드
        String[] str15 = "012345-1230123".split("-");
        ...
        //[74, 65, 86, 65, 32, 80, 82, 79, 71, 82, 65, 77] = 아스키코드

...
select customer , sum( order_qty ) as 총주문수량 from orders where customer in('김민준', '이서연') group by customer having 총주문수량 >= 4;
-- 문제 7번
select avg(price) as 평균가격 from books where stock < 5;
-- 문제 8번
# 레코드수가 3개 이상인 도서 = having
...
-- 문제 12번
select * from books; 
select sum(stock) as 총재고수량 from books where genre = '소설';
-- 문제 13번
select count(distinct customer) as 총인원 from orders;
-- 문제 14번
select * from orders; 
select customer, min(order_date) as 빠른주문일, max(order_date) as 최근주문일 from orders group by customer;
-- 문제 15번
select * from books;
select * from orders;
select book_id, avg(order_qty) as 평균주문수량 from orders group by book_id;
-- 문제 16번
select genre as 장르, avg(price) as 평균가격 from books group by genre having avg(price) > 18000;
-- 문제 17번
select * from orders;
select sum(order_qty) as 총주문수량 from orders where order_date between '2023-01-01' and '2023-03-31';
-- 문제 18번
select * from orders;
select customer, count(distinct book_id) as 도서개수 from orders group by customer order by 도서개수 desc limit 1;
-- 문제 19번
select genre, min(price) as 낮은가격 from books group by genre;
-- 문제 20번

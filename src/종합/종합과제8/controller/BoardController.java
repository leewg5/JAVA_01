package 종합.종합과제8.controller;


import 종합.종합과제8.model.dao.BoardDao;
...
public class BoardController {
    ...
    // (1) 등록 기능 구현
    public boolean boardWrite(String cphone, int cpop) {
        // 1. 여러가지 유효성검사 (패스~)
        // 2. 데이터문제 없으면 묶음(객체)하나로 만들기
        // --> 주의할점 매개변수와 동일한 생성자가 존재하지 않으면 오류 발생한다.

        BoardDto boardDto = new BoardDto(0, cphone, cpop);
    ...
    public boolean boardDelete(){
        ...
        boolean result = boardDao.boardDelete( cno );
        // 4. 결과를 view에게 리턴한다.
        return result;
    }

    // (4) 수정 기능 구현
    public boolean boardUpdate(int cno, String cphone, int cpop){
        // 1.유효성검사
